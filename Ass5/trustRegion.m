...
while ~stopCond && (k < maxIter)
  ...
  if sr1
    ...
    B=F.d2f(x_k);
    if abs((y_k-B*p)'*p)>= r*norm(p)*norm(y_k-B*p)
        F.d2f=@(x) (B+(y_k-B*p)*((y_k-B*p)')/((y_k-B*p)'*p))*eye(length(x));
    end
    %====================================================================
  end
  ...
  if sr1  
    ...
    %==================== YOUR CODE HERE ============================
    H=inv(B);
    if abs((p-H*y_k)'*y_k)>= r*norm(y_k)*norm(p-H*y_k)
        H= (H+(p-H*y_k)*((p-H*y_k)')/((p-H*y_k)'*y_k))*eye(length(x_k));
    end
