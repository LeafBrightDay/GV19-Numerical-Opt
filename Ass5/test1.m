clear;
% For computation define as function of 1 vector variable
F.f = @(x) (x(1) - 3*x(2)).^2 + x(1).^4;
F.df = @(x) [2*(x(1) - 3*x(2)) + 4*x(1).^3; -6*(x(1) - 3*x(2))];
F.d2f = @(x) [2 + 12*x(1).^2, -6; -6, 18];

% Starting point
x0 = [10; 10]; 

% Parameters
maxIter = 200; 
tol = 1e-10; % Stopping tolerance on relative step length between iterations

% Line search parameters
alpha0 = 1;

% Strong Wolfe LS
lsOpts_LS.c1 = 1e-4;
lsOpts_LS.c2 = 0.9; % 0.1 Good for Newton, 0.9 - good for steepest descent, 0.5 compromise.
lsFunS = @(x_k, p_k, alpha0) lineSearch(F, x_k, p_k, alpha0, lsOpts_LS);
lsFun = lsFunS;

% Minimisation with Newton, Steepest descent and BFGS line search methods
tic
[xLS_BFGS, fLS_BFGS, nIterLS_BFGS, infoLS_BFGS] = descentLineSearch(F, 'bfgs', lsFun, alpha0, x0, tol, maxIter)
disp(toc/(nIterLS_BFGS-1))
err=[];
nIter=[];
for loop=1:(nIterLS_BFGS-1)
    Hk=infoLS_BFGS.H{loop};
    xk=infoLS_BFGS.xs(:,loop+1);
    nIter=[nIter loop];
    err(loop)=norm(eye(2)-Hk*F.d2f(xk));
end
plot(nIter,err);
% result = [];
% %plot(infoTR_SR1.xs(1,:), infoTR_SR1.xs(2,:))
% for i = 1:length(infoLS_BFGS.xs(1,:))
% result = [result F.f(infoLS_BFGS.xs(:,i))];
% end
% plot(1:length(result), result)